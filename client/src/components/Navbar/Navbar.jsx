import { ArrowDropDown, Notifications, Search } from "@mui/icons-material";
import "./navbar.scss";
import { useContext, useState } from "react";
import { Link } from "react-router-dom";
import { AuthContext } from "../../context/authContext/AuthContext";
import { logout } from "../../context/authContext/AuthActions";

const Navbar = () => {
  const [isScrolled, setisScrolled] = useState(false);
  const [navToggle, setNavToggle] = useState(true);

  const { user, dispatch } = useContext(AuthContext);

  const handleLogout = () => {
    dispatch(logout());
  };
  window.onscroll = () => {
    setisScrolled(window.pageYOffset === 0 ? false : true);
    return () => (window.onscroll = null);
  };
  return (
    <div className={isScrolled ? "navbar scrolled" : "navbar"}>
      <div className="container">
        <div className="left">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/7a/Logonetflix.png/1200px-Logonetflix.png?20170904093427"
            alt=""
          />
          <Link to={"/"} className="link">
            <span>Homepage</span>
          </Link>
          <Link to={"/series"} className="link">
            <span>Series</span>
          </Link>
          <Link to={"/movies"} className="link">
            <span>Movies</span>
          </Link>
          <Link to={""} className="link disabled-link">
            <span>New and Popular</span>
          </Link>
          <Link to={""} className="link disabled-link">
            <span>My List</span>
          </Link>
        </div>
        <div
          className="toggle-btn"
          onClick={() => setNavToggle(!navToggle)}
        ></div>
        <div className={navToggle ? "left-hamburger hidden" : "left-hamburger"}>
          <div
            className={
              navToggle ? "hamburger-content hidden" : "hamburger-content"
            }
          >
            <img
              src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/7a/Logonetflix.png/1200px-Logonetflix.png?20170904093427"
              alt=""
            />
            <Link to={"/"} className="link">
              <span>Homepage</span>
            </Link>
            <Link to={"/series"} className="link">
              <span>Series</span>
            </Link>
            <Link to={"/movies"} className="link">
              <span>Movies</span>
            </Link>
            <Link to={""} className="link disabled-link">
              <span>New and Popular</span>
            </Link>
            <Link to={""} className="link disabled-link">
              <span>My List</span>
            </Link>
          </div>
        </div>
        <div className="right">
          <Search className="icon disabled-link" />
          <span className="kid disabled-link">Kid</span>
          <Notifications className="icon disabled-link" />
          <div className="profile">
            <img
              src={
                user.profilePic ||
                "data:image/png;base64,"
              }
              alt=""
            />
            <ArrowDropDown className="ico" />
            <div className="options">
              <span className="makeFocus" onClick={handleLogout}>
                Logout
              </span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Navbar;
